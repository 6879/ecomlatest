<template>
  <div>
    <div class="container-fluid">
      <div class="row">
        <div class="col-12 col-xs-12 col-md-5 mt-4 p-4 ml-2 profilebox">
          <center>
            <h3 class="align-center cush">
              <i class="fas fa-trash-restore-alt"></i>&nbsp; Cancel Order List
            </h3>
          </center>
          <table class="table table-striped table-responsive">
            ...
            <tbody>
              <tr v-for="(invoice, index) in invoices" :key="invoice.id">
                ...
                <td>
                  <button
                  data-toggle="modal" :data-target="'#' +'can'+ invoice.invoiceNo"
                ...
                <div class="modal fade" :id="'can'+ invoice.invoiceNo" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header" style="background-color: #ec7272;">
    ...
    <div class="col-12 col-xs-12 col-md-5 mt-4 p-4 ml-2 profilebox">
      <center>
        <h3 class="align-center cush">
          <i class="fas fa-trash-restore-alt"></i>&nbsp; Cancel Order By Order
          Management
        </h3>
      </center>
      <table class="table table-striped table-responsive">
        ...
        <tbody>
          <tr v-for="(cancelInvoice, index) in cancelInvoices" :key="cancelInvoice.id">
            <th scope="row">{{ index + 1 }}</th>
            <td>#{{ cancelInvoice.invoiceNo }}</td>
            <td>{{ moment(cancelInvoice.created_at).format("DD-MM-YYYY") }}</td>

            <td>
              <button
                 data-toggle="modal" :data-target="'#' +'cancel'+ invoice.invoiceNo"
                class="btn btn-primary"
              >
                Details
              </button>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header" style="background-color: #ec7272;color:white">
       Cancel Reason:{{cancelInvoice.cancelReason}}
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
    ...
    <div>
                <center>
                  <h5>{{ moment(cancelInvoice.created_at).format("DD-MM-YYYY") }}</h5>

                  <p class="mtop">Order No #{{ cancelInvoice.invoiceNo }}</p>
                </center>
                <table class="table d-none d-sm-block">
                  <tbody>
                    <tr
                      v-for="(order, index) in orders"
                      v-if="order.invoiceNo == cancelInvoice.invoiceNo"
                    >
                      <!-- <th >{{index+1}}</th> -->
                      <td>
                        <img
                ...
                <div class="d-md-none">
                  ...
                  >
                    ...
                    <small v-for="pnam in pnames" v-if="pnam.id == order.pId">
                      {{ pnam.pname }}</small
                    >
                    ...
                    <small
                      >qty.{{ order.qty }}&nbsp;Dis.{{
                        order.discount
                      }}TK&nbsp;CashBack.50TK&nbsp;{{ order.price }}.TK</small
                    >
                  </div>
                  <hr />
                </div>
                ...
                <div class="row">
                  ...
                  <div
                    class="col-12 col-md-4 mb-2 col-xs-12"
                    style="
                      background: rgba(225, 238, 234, 0.64) none repeat scroll
                        0% 0%;
                      color: white;
                    "
                  >
                    <table class="table">
                      <tbody>
                        <tr>
                          <td style="padding: 5px">Total</td>
                          <td style="padding: 5px">
                            <span>{{ cancelInvoice.totalPrice }} TK</span>
                          </td>
                        </tr>
                        <tr>
                          <td style="padding: 5px">Discount</td>
                          <td style="padding: 5px">
                            {{ cancelInvoice.totalDiscount }} TK
                          </td>
                        </tr>
                        <tr>
                          <td style="padding: 5px">Cash Back</td>
  ...
  methods: {
    ...
    invoiceGet() {
      axios.get("order").then((res) => {
        this.invoices = res.data.orderCancel;
        this.cancelInvoices = res.data.orderCancelByManagement;
      });
    },
    orderGet() {
      axios.get("order").then((res) => {
