  ...
  <div class="row" data-toggle="isotope">
    ...
    <div class="item col-xs-12 col-md-8">
      <div class="panel panel-default">
             <h3 class="text-h1 ribbon-heading ribbon-primary bottom-left-right">Target Sales Commission Entry</h3>  
        ...
        >
          ...
          <tbody>
            <tr v-for="(sale, index) in sales" :key="sale.id">
              <td>{{ index + 1 }}</td>
              <td>{{ sale.targetSalesAmount }}</td>          
              <td>{{ sale.link }}</td>          
              <td>{{ sale.linkAmount }}</td>          

              <td>
                <router-link
                  :to="{ name: 'edittargetsales', params: { id: sale.id } }"
...
export default {
  ...
  add() {
    ...
    viewTargetSales() {
      axios.get("targetSales")
      
        .then((res) => {
          this.sales = res.data.getTargetSales;
        })
        .catch((err) => console.log(err));
    },

    deletePost(id) {
      axios.get(`targetSales/${id}`
      ).then((response) => {
        Toast.fire({
          icon: "success",
