        ...
        >
          ...
          <tbody v-for="(order, index) in orders" :key="order.id">
            <tr>
              ...
              <td> 
                <div    v-for="dman in dmans" v-if="dman.id==order.deliveryManId">
                  <span
                  class="badge badge-primary"
            
                data-toggle="modal"
                  :data-target="'#' +'user'+ dman.id"
                >
                  {{ dman.name }}
                </span>
<!-- Modal -->
<div class="modal fade" :id="'user'+ dman.id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">DeliveryMan Information</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="panel panel-default widget-user-1 text-center" v-for="get in ddatils" v-if="dman.id==get.deliveryManId">
      
          <div class="avatar">
            <img
              :src="'adminProfileImage/'+get.image"
            ...
            <div class="expandable-content">
              <p>
                <i
                  ...
                  aria-hidden="true"
                ></i>
                <strong>Email:</strong>&nbsp;{{ dman.email }}
              </p>
              <p>
                <i
                  class="fa fa-check-circle-o"
              ...
              <p>
                <i
                  ...
                  aria-hidden="true"
                ></i>
                <strong>Address:</strong>&nbsp;{{ get.presentAddress }}
              </p>
              <p>
                <i
                  class="fa fa-check-circle-o"
                  ...
                  aria-hidden="true"
                ></i>
                <strong>Joining Date:</strong>&nbsp;{{ moment(get.created_at).format("DD-MM-YYYY") }}
              </p>
              ...
              <div class="selector">
                <select id="inputState" class="form-control">
                  ...
                  <option
                   v-if="order.status == 8||order.status == 9||order.status == 10||order.status == 11"
                    data-toggle="modal"
                  ...
                  <option
                    v-if="order.status == 8||order.status == 9||order.status == 11"
                    @click.prevent="finish(order.invoiceNo)"
                  >
                    finish
                  </option>
               
                  <option  v-if="order.status == 10||order.status == 11" class="btn btn-warning">Finish</option>
                
                 
                  <option  v-if="order.status == 11"  @click.prevent="forward(order.invoiceNo)">Forward</option>
...
export default {
  ...
  methods: {
    ...
    accepted(id) {
      axios.get(`deliveryManagement/${id}`).then((res) => {
        Toast.fire({
          icon: "success",
    ...
    finish(id) {
      axios.get(`finishOrderManagement/${id}`).then((res) => {
        Toast.fire({
          icon: "success",
    ...
    forward(id) {
      axios.get(`deliveryManagement/${id}/edit`).then((res) => {
        Toast.fire({
          icon: "success",
    ...
    viewOrder() {
      axios
        .get("deliveryManagement")

        .then((res) => {
          this.orders = res.data.newOrder;
          this.dmans = res.data.deliveryMan;
          this.ddatils = res.data.data;
        })
        .catch((err) => console.log(err));
    },
    orderGet() {
